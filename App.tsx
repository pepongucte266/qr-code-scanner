...
const Footer: React.FC = () => (
    <footer className="absolute bottom-0 left-0 right-0 p-4 text-center">
        <p className="text-xs text-light-text-secondary dark:text-dark-text-secondary">
            Mọi quá trình xử lý đều diễn ra trên trình duyệt của bạn. Hình ảnh của bạn không được tải lên máy chủ.
        </p>
    </footer>
);
  ...
  const processImageBlob = useCallback((blob: Blob) => {
    ...
    image.onload = () => {
      ...
      if (code) {
        ...
        setErrorMessage('Không thể tìm thấy mã QR trong hình ảnh này. Vui lòng thử với hình ảnh rõ nét hơn.');
        setAppState(AppState.ERROR);
      }
      URL.revokeObjectURL(imageUrl);
    };
    image.onerror = () => {
        setErrorMessage('Tệp hình ảnh bị lỗi hoặc không được hỗ trợ.');
        setAppState(AppState.ERROR);
        URL.revokeObjectURL(imageUrl);
    }
    image.src = imageUrl;
        ...
        return (
          <div className="text-center">
            <div className="p-10 border-2 border-dashed border-light-border dark:border-dark-border rounded-xl flex flex-col items-center justify-center space-y-4">
                ...
                <h2 className="text-2xl font-bold text-light-text dark:text-dark-text">Dán hình ảnh mã QR vào đây</h2>
                <p className="text-light-text-secondary dark:text-dark-text-secondary">Nhấn Ctrl+V hoặc Command+V để quét</p>
            </div>
            <div className="my-6 text-light-text-secondary dark:text-dark-text-secondary">hoặc</div>
        ...
        return (
          <div className="flex flex-col items-center space-y-6">
            ...
            <div className="flex items-center space-x-3 text-light-text dark:text-dark-text">
                ...
                <span className="text-xl font-medium">Đang quét...</span>
            </div>
          </div>
        );
      case AppState.SUCCESS:
        return (
            <div className="w-full max-w-lg bg-light-card dark:bg-dark-card rounded-xl shadow-lg p-6 flex flex-col space-y-4">
                <h3 className="text-lg font-semibold text-light-text dark:text-dark-text">Kết quả quét</h3>
                <div className="relative group p-4 bg-light-bg dark:bg-dark-bg rounded-lg text-light-text dark:text-dark-text font-mono text-sm max-h-48 overflow-y-auto">
                    ...
                    <button 
                        ...
                        aria-label={isCopied ? "Đã sao chép" : "Sao chép kết quả"}
                    >
      ...
      case AppState.ERROR:
        return (
            <div className="w-full max-w-lg bg-red-50 dark:bg-error/10 border border-error/50 rounded-xl shadow-lg p-6 flex flex-col items-center text-center space-y-4">
                ...
                <h3 className="text-lg font-semibold text-error">Đã xảy ra lỗi</h3>
